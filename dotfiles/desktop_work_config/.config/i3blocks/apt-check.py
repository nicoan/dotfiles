#!/usr/bin/python3
# https://gist.github.com/ahmadnazir/c2444d6b09fd7bb4963a13bc577d92a3
    ...
    with apt.Cache() as aptcache:
        for pkg in cache.packages:
            ...
            if isSecurityUpgrade(cand_ver):
                upgrades += 1
                security_updates += 1
                security_packages.append(pkg.name + ',' + (inst_ver.ver_str if inst_ver else '-') + ',' + cand_ver.ver_str)
                continue
            ...
            for ver in pkg.version_list:
                ...
                if isSecurityUpgrade(ver):
                    security_updates += 1
                    security_packages.append(pkg.name + ',' + inst_ver.ver_str + ',' + ver.ver_str)
                    break

    # print the number of upgrades
    if options and options.show_package_names:
        write_package_names(sys.stdout, cache, depcache)
    if options and options.show_security_package_names:
        sys.stdout.write("\n".join(security_packages) + "\n")
...
if __name__ == "__main__":        
    ...
    parser.add_option("-s",
                      ...
                      action="store_true",
                      dest="show_security_package_names",
                      help=_("Show the security related packages that are going to be installed/upgraded"))
    parser.add_option("",
                      "--human-readable",
                      action="store_true",
